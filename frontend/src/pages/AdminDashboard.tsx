...
export default function AdminDashboard() {
  ...
  const [restockModal, setRestockModal] = useState<Vehicle | null>(null);
  ...
  async function handleCreate(payload: Parameters<typeof vehicleApi.create>[0]) {
    setFormLoading(true);
    try {
      ...
      showToast(`❌ ${e.response?.data?.error ?? 'Failed to create vehicle.'}`);
      throw err;
    } finally {
  ...
  async function handleUpdate(payload: Parameters<typeof vehicleApi.update>[1]) {
    ...
    try {
      ...
      const e = err as { response?: { data?: { error?: string } } };
      showToast(`❌ ${e.response?.data?.error ?? 'Failed to update vehicle.'}`);
      throw err;
    } finally {
      setFormLoading(false);
    }
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-50 text-left">
                    <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Vehicle</th>
                    <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Category</th>
                    <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Price</th>
                    <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Stock</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {vehicles.map((vehicle) => (
                    <tr key={vehicle.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="badge-indigo">{vehicle.category}</span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <button
                            id={`admin-restock-btn-${vehicle.id}`}
                            onClick={() => { setRestockModal(vehicle); setRestockQty(''); }}
                          ...
                          <button
                            id={`admin-delete-btn-${vehicle.id}`}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="card p-6 w-full max-w-sm shadow-xl animate-slide-up">
            ...
            <p className="text-sm text-slate-500 mb-4">
              {restockModal.make} {restockModal.model} — Current: <strong>{restockModal.quantity}</strong>
            </p>
