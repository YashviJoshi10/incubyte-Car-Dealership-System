...
type ModalType = 'edit' | 'restock' | null;
...
export default function Dashboard() {
  ...
  async function handleEditSubmit(payload: Parameters<typeof vehicleApi.update>[1]) {
  ...
  async function handleRestockSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const e = err as { response?: { data?: { error?: string } } };
      showToast(`❌ ${e.response?.data?.error ?? 'Restock failed.'}`);
    } finally {
      setModalLoading(false);
    }
  }
      ...
      <main className="page-container">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex gap-3">
            <div className="card px-4 py-2.5 text-center min-w-[80px]">
        ...
        {loading ? (
          ...
          <div className="card p-12 text-center">
            <svg className="w-12 h-12 text-slate-300 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 16V6a1 1 0 00-1-1H4a1 1 0 00-1 1v10l2.55-2.55A1 1 0 016 13h1m6 3h2" />
            </svg>
            <p className="text-slate-500 text-lg font-medium">
              {isFiltering ? 'No vehicles match your search.' : 'No vehicles in inventory yet.'}
            </p>
            {isFiltering && (
              <button onClick={() => handleSearch({})} className="btn-secondary mt-4">Clear filters</button>
            )}
          </div>
        ) : (
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="card p-6 w-full max-w-sm shadow-xl animate-slide-up">
            ...
            <p className="text-slate-600 mb-1">
              <span className="font-semibold">{selectedVehicle.make} {selectedVehicle.model}</span>
            </p>
            <p className="text-sm text-slate-500 mb-4">Current stock: <span className="font-semibold">{selectedVehicle.quantity}</span></p>
